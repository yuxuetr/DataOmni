...
| CockroachDB | 25.2 | PostgreSQL | ❌ 当前版本不支持 | 7 / 21 |
...
- **TiDB 的缺口**：执行计划不可用（TiDB 不认 `EXPLAIN FORMAT=JSON`，界面显示
  ...
  没有触发器与存储过程。
- **CockroachDB 为什么不收**：结构页整页打不开（没有 `pg_get_triggerdef()`，
  而触发器与索引、外键同一批加载）；执行计划不可用；错误里没有约束名和出错位置；
  对象目录里有名字为空的条目。写入与事务那一侧用例是过的，但离「日常可用」
  还差一串目录查询的改写，不是一两处。
