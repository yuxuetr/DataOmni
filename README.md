...
| SQL Server | 2022 | SQL Server | ⚠️ 可用，有缺口 | 11 / 11（独立用例，`sql_server_smoke.rs`） |
...
- **SQL Server 在分阶段接入**：能连接（含 TLS 与 SSH 隧道）、执行 SQL（结果上限、
  ...
  在表格里改数据（一次提交一个事务）、事务控制与关闭自动提交。
  还不能：执行计划、改表结构与新建表、CSV 导入、整表与完整结果导出——这几个按钮
  在 SQL Server 连接上不出现，连接表单那一格列着同一份清单。SQL Server 的读是加锁
  读：编辑器里开着一个改过某些行的事务时，表格里提交对这些行的改动、结构页读
  未提交的改表，会在等锁 5 秒后报 1222，提交或回滚编辑器里那个事务即可。脚本里的 `GO` 分隔符还不认，语句之间用分号。`sql_variant` 与 CLR 类型
