...
| Neo4j | 5.26 LTS、2026.09 | Neo4j（`neo4j` crate，Bolt 5） | ⚠️ 查询、浏览与图，界面上还不能改 | 7 / 7（独立用例，`neo4j_smoke.rs`，两个版本各跑一遍） |
...
- **Neo4j**：连接（用户名口令或不认证；库空着就是用户的主库；TLS 与 CA；SSH 隧道）、对象树按库
  ...
  点中看属性，最多画 300 个节点。
  还没有的：界面上改节点与属性、执行计划、集群路由（`neo4j://`）。
