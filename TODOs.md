...
- [-] 完成焦点管理、键盘导航和可访问性标签
  ...
  - 未做：**对象树的键盘导航**。现在每个对象都是独立的 Tab 停靠点，单组上限
    200，意味着最多 200 次 Tab 才走得过去；也没有 `role="tree"`、没有左右键
    展开折叠。这条比标签栏大，要按 tree 模式做一套（roving tabindex + 上下
    在可见节点间走 + 左右收展 + Enter 打开）。
  - 未做：`role="tabpanel"` 与标签的关联（`aria-controls` / `aria-labelledby`）
    ——标签栏和内容区分属两个组件，要 App 那边一起改。
