...
- [-] 左侧对象树、编辑器和结果区支持拖动调整
  ...
  - 正确的规则是「高度 = min(内容高, 可用高)，内容更多时不低于 600」，
    而**纯 CSS 表达不出来**：`min-height: 600` 会让三行的结果也撑出一个
    600px 的空盒子。要做对得测内容高度——ResizeObserver 加一个纯函数
    `resultTableHeight(可用高, 内容高)`，那是单独一轮的活，留给它自己做。
