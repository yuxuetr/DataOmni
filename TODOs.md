...
- [ ] MongoDB 使用文档浏览与查询模型，不复用 SQL 表格写入模型
  ...
  - **第二阶段（改、增、删单个文档）已落地**（`fae7ae1`）：编辑框就是上面那种写法，改一个
    字段不会换掉别的字段的类型（真库用例：Long、Decimal128、整数值的 Double 原样）。
    改是整篇替换，条件是 `_id` 加「`$$ROOT` 与打开时那份完全一样」——别处改过就不覆盖、
    报「被别处改过」，草稿留着；原文档放在 `$literal` 里比，值里的 `'$price'` 不会被当成
    字段路径。两道门都反向验过（去掉比对 / 去掉 `$literal` 各红一条）。视图只读。
    真库用例共 12 条。
  - **不做乐观锁以外的并发控制**：不加版本字段、不开事务。「完全一样」这个条件不需要
    集合配合（不用约定一个 `version` 字段），而单文档写在 MongoDB 上本来就是原子的。
  - **第三阶段（结构页）已落地**（`9a4dc89`）：索引与建集合时的选项（校验规则、上限、时序），
    视图写它的定义。直接发 `listIndexes` / `listCollections` 原样显示，不经驱动的类型化结构
    （它们会丢掉不认识的选项）。`objectMenu.test.ts` 对集合的豁免已拿掉。
  - SSH 隧道在打包版上验过（2026-09-24）：关掉本机的端口转发，经跳板机连上、列集合、查文档。
    这是「总是直连」那一条在真实拓扑上的验证。
  - **导出已落地**（2026-09-24）：按生效中的条件与排序导出**全部**匹配文档，每行一个，
    即 `mongoexport` 的默认格式（`mongoimport` 直接读回）。可选 relaxed（默认，好读）或
    canonical（类型完整）——这是有真实差别的两个取值：relaxed 读回来 `Long(5)` 成了 Int32，
    正是本页一直在防的「悄悄换类型」。走现成的后台任务（进度、取消与 SQL 导出共用
    `cancel_export` 和同一个「已取消」码），`.part` 后改名、取消或失败时删掉，与
    `export_writer` 共用那套 `PartFile`。不设时限：导出要多久未知，停下靠取消。
    真库用例 2 条（共 15 条）：250 个文档跨三批游标、条件与倒序、canonical 读回与库里逐个
    相等；取消后既无目标文件也无 `.part`。反向验过（去掉取消检查 / canonical 换成 relaxed
    → 两条各红）。打包版上两种写法各导出一次，文件逐行核对过。
    只一种文件格式：JSON 数组（`--jsonArray`）与 CSV 都没有消费者——CSV 要先定「嵌套字段
    怎么摊平」，而 mongoexport 的 CSV 本来就要求显式列出字段。
  - **导入已落地**（`1360e62`，2026-09-25）：读每行一个文档的 Extended JSON（relaxed 与
    canonical 都认），和导出配成一对。发 `insert` / `update` 命令本身、`ordered: false`，
    不走驱动的 `insert_many`——两种回答都带 `writeErrors[].index`，才对得回文件行号；一行
    坏了只记它自己，同批其余照常写入（mongoimport 的做法）。模式二选一：只新增（默认）/
    按 `_id` 整份替换（无 `_id` 的先补 ObjectId）。`--jsonArray` 的文件开头就拒绝。
    走现成后台任务，暂停与取消和 CSV 共用；没有事务，所以不给「重试」。批上限 1000 个 /
    8 MB 文字（命令上限 16 MB，留一半余量）。
    真库用例 4 条（共 19 条），反向验过：`ordered: true` → 撞上 `_id` 后同批其余没写，红；
    去掉取消检查 → 红；替换模式不补 `_id` → 得到 `_id: null`，红（第一版断言只数条数，
    这个变异是绿的，补了「是 ObjectId」的断言才红——`extend` 会保住原来的 `_id`，
    「总是补」那个变异与原代码等价，不算数）。打包版上两种模式各导一次：坏行与重复 `_id`
    按行号进日志，Long 类型保住，替换是整份不是合并。
    **不做**：字段映射与预览（文档自带字段名，读法只有一种）；JSON 数组（要么整份进内存，
    要么手写流式数组解析，没有观察到需求——导出这边也只出每行一个）；CSV（同导出，要先定
    嵌套字段怎么摊平）。重估条件：有人拿着 `--jsonArray` 的文件来导。
  - **建与删索引已落地**（2026-09-25）：结构页上。新建是「键」「选项」两格，即 mongosh 的
    `createIndex(keys, options)`，带命令预览——不做勾选框，因为键不只升降序（text、2dsphere、
    hashed、通配），选项是开放文档，而结构页显示的正是这种写法。发 `createIndexes` 本身，
    选项原样并入，不经驱动的 `IndexOptions`；无名按 mongosh 规矩起名。服务端对「一模一样的
    已存在」只回 ok 加 note，这里报 `MONGO_INDEX_EXISTS`。`maxTimeMS` 照查询超时给。
    真库用例 1 条（共 20 条），反向验过（不并入选项 / 去掉「已存在」判断各红）。打包版上
    建了唯一 + 部分索引、重复建看到提示、建了 text 索引、删掉，中英文都看过。
    **不做**：`hidden`、`collMod` 改 TTL 这类「改索引」——没有消费者，要改就删了重建。
  - **按条件批量改与删已落地**（2026-09-25）：条件栏右边两个按钮，只作用于**已生效**的条件
    （框里改了没查询时按钮灰掉——动的必须是网格上看到的那一组）。对话框即确认：条件、打开时
    的个数（冻结，不随写完后的重读变）、真正发出的命令，空条件红字写「整个集合」。
    改：操作符或管道；整篇替换在本地就拒（服务端的拒绝与「中途被挡」同为 writeErrors，分不开）。
    **实测发现**：multi update 被校验规则中途挡住时服务端回答 `n: 0, nModified: 0`，而前面
    处理过的文档已经改了。所以停下时不报个数，只说「前面的已经生效」并点名挡住的 `_id`；
    用例把「库里确有部分生效」钉住，服务端哪天改成报真实个数时这条会提醒重估。
    真库用例 2 条（共 22 条），反向验过（去掉本地操作符检查 / 忽略 writeErrors 各红）。
    打包版上改、再改一次（看到「本来就是这个值」）、删、替换写法被拒都看过，中英文都看过。
    **不做**：按环境（生产）加额外一道门——对话框本身已是带个数与命令的确认；有人要求
    生产库上批量写再多一道时重估，现成的判据是 `connection.environment`。
  - **只读聚合管道已落地**（`db6aec4`，2026-09-25）：集合页「查询 / 聚合」切换，结果进同一张
    网格。`$out` / `$merge` 解析时就拒。翻页在末尾接 `$skip` / `$limit n+1`，每页重跑；不数总数。
    结果行**不按 `_id` 回库取**：`$group` 的分组键可能等于某个真文档的 `_id`，那样会打开并能改
    另一个文档——全文随页带回，面板只读。生效的查询多一个 `pipeline` 字段，`load` 一处分叉，
    其余调用点不动；导出与批量改删跟着网格里显示的走。真库用例 1 条（共 23 条），反向验过
    （不接 `$skip` → 第二页重复第一页，红）。打包版上分组、翻两页、点结果看全文、`$out` 被拒
  ...
  - **X.509 登录已落地**（2026-09-25）：「认证方式」口令 / 客户端证书，后者存 `options.authMechanism
    = MONGODB-X509`。凭据只给机制与 `$external`，不带用户名口令（表单上残留的也不发、不进键）；
    SRV 那条路上 TXT 的 `authSource` 被它盖掉。键落在 `$external`，与同一台主机上不带凭据的
    连接不共用池子。没证书或没开 TLS 就拒（`MONGO_X509_NEEDS_CERTIFICATE`，否则服务端只回
    「认证失败」）；切到证书时口令格清空，不把看不见的字存进钥匙串。真库用例 1 条（共 29 条）：
    登上的身份就是证书主题 `O=DataOmni,OU=test,CN=dataomni-client`（注意 RFC 2253 顺序，
    第一次建用户时写反了就登不上）；同一个 CA 签的 `stranger` 没有用户 → 认证失败。
    反向验过：不给 X.509 凭据 → 红。
    **不做**：LDAP（PLAIN）、Kerberos（GSSAPI）、AWS IAM——前两种要企业版服务端，这里验不了；
    AWS 要 Atlas 与 IAM 角色。重估条件：有人拿着这种账号来。
  - **建集合、删集合与视图已落地**（2026-09-25）：对象树头部「+」→ 库 / 名字 / 选项三格（即
    `createCollection(name, options)`，发 `create` 本身、选项原样并入，不经驱动的类型化选项），
    带 mongosh 命令预览；库名可填新的（MongoDB 建库就是建第一个集合）。右键「删除」发 `drop`，
    走对象树原有的确认框（`alwaysAsks`），框里是等价的 mongosh 命令。命令文字在
    `utils/mongoCommandText.ts`，两个旧对话框里各一份的引号转义也收进去。真库用例 1 条（共 30 条）：
    上限与校验规则真的落在 `listCollections` 的 options 里、同名再建报 `NamespaceExists`、新库被
    建出来、删后对象树里没有。反向验过：不并入选项 → 红。前端在浏览器里（模拟后端）看过对话框、
    右键菜单与确认框，并核对发出的参数；打包版待屏幕解锁后再看。
    **不做**：删库（对象树没有库一级的右键菜单，要先加这个入口；删库的后果比删集合大一级，
    没有被要过）、改集合名（`renameCollection` 要 admin 库上的权限，且改名后已打开的标签要跟着改）。
    重估条件：有人要。
  - **执行计划已落地**（2026-09-25）：`explain` + `executionStats`，条件与排序或管道都行，看的是
    框里的草稿（不写库，不必先生效）。后端摘出计划链与四个计数，认三种形状：经典 `winningPlan`、
    8.0 的 SBE `winningPlan.queryPlan`、聚合的 `stages`（头一个 `$cursor`）。结论只分三种
    （`utils/mongoExplain.ts`）：有 COLLSCAN 就算扫全集合（`$or` 一支走索引另一支不走时，慢的是那一支）。
...
- [ ] Redis 使用键空间、类型和值浏览模型
  - **第一阶段（只读浏览）已落地**（2026-09-25）。驱动 redis-rs 1.7（rustls），`ConnectionManager`
    多路复用、断了自己重连。
  - **一个库号一条连接**（`RedisPool`，第一次用到时才开）：`SELECT` 改的是整条连接的状态，在多路
    复用的连接上切库，同时在跑的另一条命令就落到别的库里。
  - 键和值都是字节串：往前端送 `RedisBytes`（base64 原样 + 给人看的文字 + 是否二进制），前端回传的
    永远是 base64。不是 UTF-8 的按 redis-cli 写法转义（`\xff`）；512 KiB 的字符串预览截在多字节
    字符中间时先去掉那半个字符再判断，不然一段中文会被当成二进制（单测反向验过）。
  - 只用 `SCAN` 族。一页凑够就停，但一次 `SCAN` 多给的不丢——丢了下一页再也看不到它们；过了半个
    时限先交出已有的、带着游标。类型与剩余时间一次流水线取回。游标按字符串传（可以大过 2⁵³）。
  - 值按类型分页：hash / set 用 `*SCAN` 游标，list / zset 用下标，stream 用上一页最后一条的 ID 加
    `(`（不含它自己）。翻页位置对前端是不透明的字符串，合并页是纯函数 `appendValuePage`。
  - 对象树一行一个逻辑库（新对象种类 `keyspace`，`Record` 逼着每张表都补一条）：数的是
    `INFO keyspace`，不用 `CONFIG GET databases`——它在 ACL 里算危险命令，只读用户多半没有。
    配置里那个库哪怕是空的也列出来。
  - 驱动默认的响应时限是 **500 毫秒**：经 SSH 隧道、几条用例并行时第一版全部超时。改成不设响应
    时限、每条命令外面套查询超时；连不上的重试从 6 轮退避改成 1 次，10 秒内报出。
  - 真库用例 7 条（`redis_smoke.rs`，Redis 7.4）：错口令 / 没口令 / 关着的端口各一种码；列库（空的
    配置库也在）；按游标翻完 251 个键不重不漏、类型过滤、剩余时间；六种类型各自翻页不重不漏；
    二进制键拿 base64 取回、60 万字节的中文字符串截断后仍是文字、没了的键；只读 ACL 用户能浏览；
    TLS 四档（带 CA 校验、只加密不校验、不给 CA 必须拒、明文敲 TLS 端口）加 CA 文件路径报错。
    反向验过五个变异各红：恢复默认响应时限（5 条红）、一页多出的键截掉、流翻页含上一页最后一条、
    TLS 一律不校验。浏览器里（模拟后端）看过六种类型、二进制键与值、剩余时间、中英文与连接表单；
    打包版待屏幕解锁。
  - **命令行已落地**（2026-09-25）：Redis 的命令本身就是一行文字，不需要像 mongosh 那样嵌 JS 引擎。
    前端按 redis-cli 的 `sdssplitargs` 规矩拆成**字节串**（`"\xff"` 是一个字节），base64 过去；
    后端直接 `req_packed_command`，服务端的错误回答是一条正常的回答（`(error) …`），网络与超时才是
    失败。回答照 redis-cli 画（嵌套数组按序号宽度缩进）。拒跑两类：会阻塞的（`BLPOP`、订阅、
    `MONITOR`、带 `BLOCK` 的 `XREAD`）与改连接状态的（`SELECT`、`MULTI`、`AUTH`、`CLIENT REPLY`）——
    连接是各处共用的一条多路复用连接。先确认三类：清数据、`KEYS`/`DEBUG`、改服务端；其余 Redis
    写命令太多，都问等于都不问。跑过命令切回键列表会重扫。真库用例 1 条（共 8 条）：二进制参数
    原样写进库 12、错误回答、nil、数组、`SELECT` 被拒且之后仍在库 12。反向验过：去掉拒绝 →
    `SELECT 0` 真把共用连接切走，之后的 `EXISTS` 红。
  - **键级改动与改字符串已落地**（2026-09-25）：删（`UNLINK`）、改名（`RENAMENX`，已有就报
    `REDIS_KEY_EXISTS` 不覆盖）、设 / 去过期、改字符串。「别处改过就不覆盖」用一段 Lua 先比对再写
    （多路复用连接上没法 `WATCH`，脚本在服务端原子执行），`KEEPTTL` 保住剩余时间。每一种碰到键已经
    没了都报 `REDIS_KEY_GONE`，不悄悄建出一个新的（`PERSIST` 回 0 分不清「没过期」和「不在」，另问一次
    `EXISTS`）。截断的与二进制的字符串不给编辑；二进制的键不给改名（框里的转义文字写不回原样字节）。
    真库用例 1 条（共 9 条）；反向验过：去掉比对 → 覆盖了别处的改动，红；去掉 `KEEPTTL` → 剩余时间变
    -1，红。同时发现并修掉一处用例之间的干扰：并行跑时两条用例共用库号，一条的 `FLUSHDB` 会清掉另一条
    的数据——改成每条一个库号（8–15），整套连跑三遍都绿。
  - **改元素与新建键已落地**（2026-09-25）：hash 加 / 改 / 删字段，list 改 / 删项与推到头尾，set 加 / 删，
    zset 加 / 改分数 / 删；新建六种键（带第一个元素与可选过期）。每种一段 Lua：先看键在不在
    （`HSET`、`SADD`、`ZADD` 对不在的键会建出一个新的，而人以为自己在改的那个已经没了）、再比对。
    list 按下标删没有现成命令：比对后把那一格换成一次性记号再删这个记号——按值 `LREM` 删的是第一个
    同值的那一格。脚本回答统一成五种（写了 / 值变了 / 键没了 / 已经有 / 元素没了）。分数格式在
    `isRedisScore` 里先挡（含 `inf`）。真库用例 1 条（共 10 条）；反向验过：`SET_ADD` 不先看键 → 把一个
    已经没了的键建了出来，红；按值删 → `[p, q, p, r]` 删第 2 格得 `[q, p, r]`，红。浏览器里（模拟后端）
    核对了每种改动发出的参数，并修掉两处：添加失败时那一行被清空（`submit` 吞掉了失败，现在返回成败）；
    分数不合规矩时聚焦的那一格不变红（两种 ring 颜色同时写，谁赢看样式表次序）。
  - **还没做**：stream 的写入（`XADD` / `XDEL`，命令行里能做）；集群（按槽路由，是另一种连接）、
    Sentinel、客户端证书——重估条件：有人拿着这种部署来。
  - **这一版不做**：集群（要按槽路由，是另一种连接）、Sentinel、客户端证书（redis-rs 要证书与私钥
    的字节，表单是路径，要先定读文件的规矩）、RESP3 推送与发布订阅。重估条件：有人拿着这种部署来。
- [x] Neo4j 使用 Cypher 和图结果模型（五个阶段都已落地，2026-09-25）
  - **2026-09-25 开工。驱动先做实验再选**：cu 上起 Neo4j 5.26.31（LTS）与 2026.09.0（最新），
    三个 Rust 驱动各跑一次握手加一条返回节点的查询：
    - `neo4rs` 0.8（稳定版）握手只报 Bolt 4.0 / 4.1，两个版本的服务端**居然都接受**；0.9 只是
      rc，到 4.4 为止。Bolt 4 的节点**没有 `element_id`**（Neo4j 5 起的标识，`id()` 已弃用），
      行拿不到有序的列名，结果摘要（计数、计划）在 0.9 里还挂在 `unstable-*` feature 后面。
    - `neo4j` 0.2（作者是 Neo4j 官方 Python 驱动的维护者，个人项目、按官方驱动的设计写、跑官方
      testkit）协商到 **Bolt 5.8**：有序列名、动态值枚举、带 `element_id` 的节点、完整摘要
      （服务端版本、计数、计划）；TLS 三档（系统 CA / 自带 CA / 不校验）、连接超时、服务端事务
      超时、按库执行都有。缺点是**同步 API**，放进 `spawn_blocking`。
    - 选 `neo4j` 0.2。一个数据库客户端要的正是列的次序、任意类型的值、稳定的标识和计划。
      重估条件：它停更且某个新服务端版本不再接受 Bolt 5.8 以下（握手失败的用例会红）。
  - **分阶段**，每一阶段都能用、都提交：
    1. 连接（直连 `bolt://`；用户名口令或不认证；库空着就是用户的主库；TLS 与 CA；SSH 隧道）、
       对象树（每个库一组，下面是标签与关系类型）、Cypher 查询标签（复用 `sql` 种类的标签：草稿、
       去重、持久化都现成）：多语句按 `;` 拆开依次跑，结果表的值按 Cypher 字面量写
       （`(:Person {name: 'a'})`、`[:KNOWS]`、`date('2024-01-01')`），结果上限、超时、写入计数。
    2. ~~标签 / 关系类型的浏览页~~——**第一阶段做完后撤掉**（2026-09-25）：点标签已经开出一个
       跑过的 `MATCH (n:标签) RETURN n LIMIT 25` 查询标签，条件、排序、分页都是在那条查询上改几个字，
       另做一页带 `WHERE` / `ORDER BY` 框的，只是把同一件事换一种写法再做一遍。
       重估条件：有人要不写 Cypher 就能翻页、筛选（比如给不写查询的人用）。
    3. ~~图：结果里有节点与关系时画成图（手写 SVG，与 ER 图同一套做法，节点数设上限）。~~ 已完成（见下）。
    4. ~~在界面上改：属性、标签、删节点（带关系的要确认 `DETACH`）、建节点——从结果里点中的节点改。~~ 已完成（见下）。
    5. ~~`EXPLAIN` / `PROFILE` 的计划树。~~ 已完成（见下）。
  - **第一阶段已落地**（2026-09-25）：
    - 后端 `services/neo4j.rs`：驱动放进 `spawn_blocking`；每条查询带**服务端事务超时**。
      反向验过为什么非它不可：去掉之后 20 亿行的查询在服务端一直跑，本机 6 秒放弃了，服务端
      136 秒后才停，下一条查询排在它后面；而 cu 上那台 768 MB 的容器就是这次被 OOM 杀掉的。
    - 行读到上限就 `consume`，服务端丢弃其余的：五千万行、上限 5 行，1.2 秒回来。
    - 值是带 `kind` 的结构（节点带 `elementId`、路径按走向带方向、64 位整数与浮点按字符串），
      时间与空间在后端写成 Cypher 构造写法；属性按名字排序（驱动给的是 `HashMap`）。
      打包版上发现并修掉一处：serde 的 `rename_all` 只改变体名、不改变体里的字段，前端读
      `elementId` 读到 `undefined`（单测补上，去掉 `rename_all_fields` 即红）。
    - 连接时在配置的库里跑一条 `RETURN 1`：口令错、库名错（`DatabaseNotFound`）在连接这一步报出来。
    - 对象树：`SHOW DATABASES`（system 库上，只列这个用户看得到的、在线的、不含 system），
      没权限列库时退回配置的库或主库（`db.info()`）；每库 `db.labels()` 与
      `db.relationshipTypes()`。新对象种类 `label`、`relationship-type`；点开是一条查询
      （`browseQuery`），开出来就跑一次；右键复制成 `:Person`。
    - 查询标签：复用 `sql` 种类的标签（`hasQueryEditor` 与 `speaksSql` 分开：ER 图、建表仍只问
      后者），里面画 `CypherWorkbench`。按 `;` 拆语句（Cypher 的字符串用反斜杠转义、不认 `''`，
      反引号名字里 `` `` `` 是一个反引号——写用例时就撞上了这一条）；一条失败就停。
    - 先确认：一个会写的词都没有的直接跑；有的先 `EXPLAIN` 问服务端类型（`r` 就是读，哪怕字面上
      有 `CALL`），再按关键字分级，门槛沿用「危险语句确认」。确认框的「批量写」换成 Cypher 的说法。
    - 真库用例 7 条（`neo4j_smoke.rs`），5.26.31 与 2026.09.0 各跑一遍全绿：错口令 / 关着的端口 /
      不存在的库；十六种值的写法；节点、关系、反向路径的 id 与方向；早停不整份拉；服务端超时与
      之后连接仍可用、语法错带行列；`EXPLAIN` 分得清 r / rw / w / s 且不执行；对象树。
      反向验过：路径方向取反、去掉事务超时，各自红。
    - 打包版上走过：连接、对象树、点标签自动跑、多条依次跑与写入计数、删除先确认（取消后服务端
      节点数不变）、语法错、看一格的全文与 elementId、中英文。修掉的另三处：⌘↩ 跑查询时编辑器
      里多出一个空行（CodeMirror 默认把 Mod-Enter 绑成插入空行、且先于 React 的 onKeyDown——改绑
      在编辑器键位表里、最高优先级）；对象树头部冒出 MongoDB 的「新建集合」；确认框写「没有 WHERE」。
  - **第三阶段（图）已落地**（2026-09-25，`30e237e`）：
    - 判断全在 `utils/cypherGraph.ts`（29 条测试），组件 `CypherGraphView` 只画和处理拖动。
      结果里有节点、关系或路径时默认画成图，结果段右上角「图 / 表格」切换；纯标量的结果没有切换。
    - 收：按 elementId 去重、钻进列表 / 映射 / 路径；只 `RETURN r` 时端点补成只有 id 的虚线圈。
    - 排布：力导向，没有随机数（葵花籽起点），同一份结果每次一样。**斥力按距离截断**是量出来的：
      不截断时 n 个孤立点铺开的半径约为 强度 × √(n / 收力)，`MATCH (n:L) RETURN n LIMIT 25`
      这种最常见的全孤立点结果会隔一两百像素散开；截断后 20 个孤立点最近隔 99。最后几轮把叠着的
      硬推开（打包版上两个城市各连六个人，被拉到一处叠成一团——补了用例，去掉推开这步即红）。
    - 颜色：按节点身上**在这份结果里最少见**的标签，不按第一个。打包版上发现的：`(:Movie:GraphDemo)`
      回来是 `[GraphDemo, Movie]`，电影全被涂成那个人人都有的标记标签。八个槽位不取模。
    - 同一对节点间的几条关系各弯各的，A→B 与 B→A 一左一右；自环画在圈上方，一个比一个大。
    - 不接滚轮缩放：图嵌在上下滚的结果列里，滚轮被吃掉就翻不到下一段。缩放走按钮。
    - **上限 300 个节点**，门是 `cypherGraph.test.ts` 比字面量 300（反向验过：改成 301 即红）。
      量过：排布 300 点 51ms、500 点 133ms、1000 点 541ms（Node）；打包版上拖动 300 点 / 328 线
      每帧重画中位 2ms、最长 4ms——拖动不是瓶颈，卡住上限的是排布按平方涨，以及 300 个圈适应到
      480px 高的框里时已缩到 26%、名字读不出来。超出的注明少画了几个节点、几条关系。
      重估条件：有人要在图上看几百个以上的节点（那时要的是聚合或按需展开，不是把上限调大）。
    - 反向验过：去掉斥力截断、弯线不按走向换算、着色取第一个标签，各自精确变红。
    - 打包版上走过：路径结果（19 点 46 线）、点节点 / 点关系看属性与高亮、拖节点线跟着动且不算点击、
      拖空白平移、400 点的截断提示、只返回关系、多条语句各一段、切表格、浅色主题、英文。
  - **第四阶段（在界面上改）已落地**（2026-09-25，`e898d58`）：
    - 语句全在 `utils/cypherEdit.ts`（20 条测试），组件 `CypherEntityEditor` 只管编辑框里的样子。
      点中的节点或关系（图上、表格里都行）下面就是它的编辑框；工具栏「新建节点」用同一个框。
    - **属性的值按 Cypher 表达式写**，输入框里放当前值的字面量：Neo4j 的属性不声明类型，类型只能
      从写法里来，原样不动就是不改。字面量要能原样写回去——服务端上验过 `toFloat('NaN')` /
      `'Infinity'` 读回来就是那三个值（没有字面量），`1e17` 是浮点；顺手修了后端把 `1e20` 写成
      `100000000000000000000`（读回来是整数且越界，`d87334c`）。字节串写不成，只能删。
    - 按 `elementId` 定位，一条只动一个；`REMOVE` 写在 `SET` 前——把 `a` 改名 `b`、又新加一个 `a`
      时，先设后摘会摘掉新加的那个。以 `RETURN` 拿回改完的样子，结果里每一处（含路径里的）换掉；
      草稿按新样子从头来（不然改过名的那行会把新键当成要删的）。
    - 删：先数连着几条关系（无向模式，自环服务端只算一条，验过），确认框写明；**只在确认过之后才
      `DETACH`**。打包版上验过为什么：点下删除、确认之前在 cypher-shell 里给它连一条关系，确认后
      服务端报 `ConstraintValidationFailed`、那条关系还在——若一律 `DETACH`，就悄悄删掉了一条没人
      告诉过你的关系。删完拿掉提到它（或连着它的关系）的行。
    - 别处已删掉的：保存、删除都报「没找到它」（`MATCH` 不到就没有行），不会改到别的东西上。
    - 改按「有界的写」、建按「追加」过危险语句确认，删每次都确认；写回结果来自的那个库
      （`elementId` 带着库的标识，写到别的库上只会找不到）。
    - 反向验过：`SET` 挪到 `REMOVE` 前、删一律 `DETACH`、删节点时不管连着的关系，各自精确变红。
      另有一道护栏（写不成字面量的原值锁住键）变异后没红——那一行本来就在写之前跳过，删掉了。
    - 打包版上走过并修掉的：语句预览随输入出现时面板往上长、输入框在光标底下挪位置（改成固定
      高度）；刚点「加属性」就报「键空着」（全空的新行不算数）；建出来的段在看不到的地方（渲染后
      按它的位置滚过去，不用会连外层一起滚的 `scrollIntoView`）；确认框写「从对象树发起的删除」；
      `formInputs.test.ts` 那道门拦下三个没带 `PLAIN_TEXT_INPUT` 的框——macOS 会把 `'` 换成弯引号，
      正好毁掉 Cypher 表达式。走过：建节点（标签、`date`、字符串）、加标签、`toFloat('NaN')`、
      改关系的键与值（路径与表格里跟着换）、删带自环与出边的节点、删到一半被连上关系、已被删掉、
      门槛调高后建节点先确认（设置已改回默认）、英文、浅色主题。
    - **这一版不做**：在界面上建关系（要在图上选两个端点，一条 `CREATE` 写起来更快）。
      重估条件：有人要不写 Cypher 就能连节点。
  - **第五阶段（执行计划）已落地**（2026-09-25，`3b59784`）：
    - 先做实验：驱动的摘要里带 `plan` / `profile` 两棵树。算子名带 `@neo4j` 后缀；参数里 `Details`
      是这一步动的东西、`EstimatedRows` 是估算，`PROFILE` 另有行数、`DbHits`、内存、页缓存；根节点的
      `string-representation` 是服务端排好的那张表。社区版 slotted runtime 每步的 `time` 恒为 0、
      参数里也没有 `Time`——所以**不当 `actual_ms`**，显示出来会被读成「0 毫秒」。
    - 后端转成与 SQL 执行计划同一个 `PlanNode`（`Details` 当 target，估算 / 实际对上），其余参数
      原样进 detail、`DbHits` 排第一；服务端的表进「计划原文」。挂在结果摘要上，不另开命令：
      用户在查询前面写 `EXPLAIN` / `PROFILE` 就是 Neo4j 的做法，一次执行、一份结果。
    - 结果段有计划时多「计划 / 计划原文」，默认看计划；`EXPLAIN` 没有行，不给一张空表。计划树
      从 `QueryPlanDialog` 抽成 `PlanTree` 两处共用，「估错最多」的提示照样成立（估 10、实际 5000 标红）。
    - `cypherPreamble` 认出开头的 `CYPHER` 选项与 `EXPLAIN` / `PROFILE`（次序随意、可有注释，查询里
      叫 `explain` 的变量、字符串与反引号名字里的不算）。`EXPLAIN` 不执行：不算写、不走确认——此前
      `EXPLAIN MATCH (n) DETACH DELETE n` 会按批量删除弹框。`PROFILE` 问查询类型时摘掉这个词：
      服务端不接受 `EXPLAIN PROFILE`（验过：报 conflicting execution mode；`EXPLAIN EXPLAIN` 倒是接受）。
    - **打包版上撞见并修掉的**：`PROFILE` 的行数超过上限时，读到上限就 `consume`，服务端报
      `This result has not been materialised yet. Iterate over it to get profiler stats`。改成 `PROFILE`
      时把其余的行读完再丢（`readAll`，内存里仍只留上限那么多），截断提示相应改成「读完就丢了」。
      不能「出错再重读」：那会把写入再执行一遍。真库用例钉住两侧：不读完 → 报这个错；读完 →
      10 行、截断、根节点实际 5000 行。
    - 真库用例 1 条（共 8 条），5.26.31 与 2026.09.0 各跑一遍全绿；单测变异过（不传实际行数即红）。
      打包版上走过：`PROFILE` 的树与展开细节、计划原文、表格、多条语句各一段、带 `CYPHER` 选项的
      `EXPLAIN … SET`（服务端上确认没设上）、`EXPLAIN … DETACH DELETE` 不弹框且节点数不变、英文、
      浅色主题；SQL 那边的执行计划对话框（SQLite）照常。
    - **这一版不做**：工具栏上的「执行计划」按钮——在前面写一个词就是 Neo4j 自己的用法，按钮只是替
      人打这个词；重估条件：有人要不改查询文本就看计划。
  - **SQL 编辑器同样中招，已修**：光标在最后一行末尾按 ⌘↩，先插空行、光标落到空行上，「运行光标
    所在的那条」什么也没跑（有好几条时跑的是下一条）。打包版上复现过；两个编辑器改用同一个
    `runShortcutKeymap`，渲染用例反向验过（不装它，默认键位表插空行、处理函数没被调用）。
  - **五个阶段之后仍没做的**：Cypher 的执行不进查询历史（历史收的是 SQL 执行记录 `QueryExecution`）；
    不能取消在跑的查询（同步驱动，靠服务端超时停）；没有补全。
  - **这一版不做**：集群路由（`neo4j://`，按路由表连读写成员）。重估条件：有人拿着集群或
    要求路由的部署来；AuraDB 单实例用 `bolt+s://` 直连就能连。
...
- [ ] P0-P3 无阻塞级缺陷
  ...
  - **2026-09-24 macOS 打包版逐库回归**（MySQL、MariaDB、TiDB、PostgreSQL、
    ...
    - **侧边栏「删除连接」在打包版里删不掉**（`6541fc3`，做 MongoDB 时撞上的）：
