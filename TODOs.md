...
- [ ] P0-P3 无阻塞级缺陷
  ...
  - 回归里看到、**没有修**的（都不挡用，留着按需做）：
    - 一次没能复现：MariaDB 连着被停掉后，新建连接表单里测试 TiDB 卡在「Testing…」
      80 秒以上，后端线程全空闲。重启应用后同样步骤复现不了。重估条件：再出现一次，
      先 `sample` 进程、看 `DbInstances` 的读写锁是谁拿着
    ...
    - 行编辑里各格编辑框很窄、值被截；从「写入方式」选「值」后光标不进输入框；
      结构编辑填名字时顶部冒出的提示横幅把整张表往下推一行；测试连接时保存按钮也显示
      「Saving…」；MySQL 计划树里 cost_info 占一行；对 SELECT 也写「Rows affected」
  - 还不能勾：上面那条没复现的卡死要先有结论；Windows 没验。
- [x] 核心数据库兼容性矩阵和用户文档完整
