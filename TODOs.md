...
- [ ] P0-P3 无阻塞级缺陷
  ...
  - **2026-09-24 macOS 打包版逐库回归**（MySQL、MariaDB、TiDB、PostgreSQL、
    ...
    - 驱动原话（`393ac8a`）：目录查询走执行查询那条错误归类，断线 / 取不到连接有码有文案；
      CockroachDB 触发器目录按 42883 说成「这个服务端列不出触发器」
    - 确认框写「on mysql」（`77d4e84`）；测试时保存按钮写「Saving…」（`38d6428`）；
      行编辑框窄、换成「值」后光标不进去（`29a8c6b`，带渲染用例）；结构编辑横幅推表
      （`0ddcb24`）；SELECT 也写影响行数（`89fd941`）
    - 行编辑时日期时间格的选择器被截成 `01/05/20`（`aeaa3b7`）：选择器宽度是固有的，
