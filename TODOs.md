...
- [-] 未保存标签关闭时提供保存、丢弃、取消
  - 丢弃 / 取消完成于 5c88bd5：关闭带草稿的 SQL 标签先弹确认，判据为
    `selectSqlDocumentHasUnsavedContent`（按 trim 后的草稿文本判空，刚新建的空标签不拦）。
    标签栏脏点改由 `documents` 计算，此前 `tab.dirty` 对 SQL 标签从未更新过。
  - **保存暂缺**：当前没有任何可保存的去处。写 `.sql` 文件需要 `plugin-fs` 或一个 Rust
    写文件命令（`plugin-dialog` 只给文件选择器），属于 P2.3「支持保存和打开 .sql 文件」；
    「保存草稿」则需要 per-tab 持久化，属于下面的 P2.1-E。两者任一落地后把保存加回来，
    本条才能转 `[x]`。
