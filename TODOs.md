...
- [ ] 后端还有一批中文错误串会原样印到英文界面上
  ...
  - 判据：`utils/backendError.test.ts` 那道门是从 Rust 源码里正则取
    `pub const X: &str = "DATAOMNI_…"` 的，所以**加码不配文案会红**；
    但它扫不到还没改成码的中文串。补这一批时把门扩成「`Err("中文")` 一条
    都不许有」。
...
- [x] 定义冷启动、切换标签、展开对象树和滚动性能指标
  ...
  - 量法（四条都是同一套）：触发交互，用 `requestAnimationFrame` 轮询等到节点
    ...
    帧间隔分布。
- [ ] 切换回表标签要重新打 7 次库，把它降下来
  - 量「切换标签」时发现的（扣掉 React StrictMode 的重复调用后）：切回一个表
    标签会重新发 `get_schema_metadata_queries` ×2、列目录、数据查询、行数统计、
    索引、外键、触发器——**7 次往返**。前端画那 200 行只要 109.5ms，而这 7 次
    往返在一条 30ms RTT 的远程库上就是 200ms+，大表上的 COUNT 还可能是几秒。
  - 起因是表标签切走就卸载，`tableSchema`、`schemaObjects`、`rowCountCacheRef`
    全是组件状态，回来时一切从头再来。
  - 不能简单地「数据页不查索引」：索引要喂 `rowIdentity`，数据页靠它决定一行
    怎么定位才能改。真正的缝在缓存——`appStore.databaseMetadata` 已经是同一个
    形状（按连接缓存 + `schemaVersion` 失效），表结构可以照着做一份。
  - 留到它自己那一轮做：这条是在「定义指标」时量出来的，不该塞进那一条里。
